  ...
  return (
    <div className="container mx-auto p-4 bg-sky-900 rounded-2xl">
      ...
      <div className="flex flex-wrap justify-center gap-6 mt-6">
        {cours.length === 0 ? (
          <p className="text-white text-center w-full">Aucun cours disponible pour le moment.</p>
        ) : (
          cours.map((cours) => (
            <div
              key={cours._id}
            ...
            >
              ...
              >
                {hoveredCourse === cours._id ? (
                  <div className="relative z-10 flex flex-col items-center bg-amber-50">
                    <p className="mb-2 overflow-auto max-h-24">{cours.description || "Aucune description."}</p>
                    <div className="text-yellow-400 text-xl select-none flex items-center gap-2">
                      <h2 className="text-base font-semibold text-black m-0">Niveau :</h2>
                      <span>
                    ...
                    <div className="text-yellow-400 text-xl select-none flex items-center gap-2">
                      <h2 className="text-base font-semibold text-black m-0">Prix :</h2>
                      <span>GRATUIT</span>
                    </div>
                  </div>
                ) : (
              ...
              <h2 className="mt-3 text-lg font-semibold text-yellow-400">{cours.name}</h2>
