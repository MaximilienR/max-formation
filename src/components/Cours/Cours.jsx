...
function Tableau() {
  ...
  return (
    ...
    <h1 className="text-3xl text-center mt-4 font-bold text-yellow-400 ">Nos formations</h1>
    ...
    <div className="flex flex-wrap justify-center">
        {cours.map((cours) => (
          <div key={cours.id} className="w-full md:w-1/3 xl:w-1/3 p-4">
            <img src={cours.image} alt={cours.nom} className="w-1/2 h-1/2 object-cover aspect-square" />
            <Link to="/contenu">
              <button className="bg-yellow-400 text-black font-bold py-2 px-4 rounded">En savoir plus</button>
            </Link>
          </div>
        ))}
      </div>
